...
### نشست ۵۸ — 2026-08-26 — 🎙️ حالت صوتی — رسانه و وب ۳/۵
- ترجمه و انتشار «حالت صوتی» (۸ بخش): پیش‌نیازها و پوشش Nous Portal، جدول سه قابلیت ‏(صوت تعاملی/پاسخ صوتی خودکار/کانال صوتی)، extras های pip و وابستگی‌های سیستمی، کلیدهای API با STT صفر-کلیدی faster-whisper، حالت CLI با حلقهٔ ضبط Ctrl+B و آشکارسازی دومرحله‌ای سکوت، پایان با عبارت stop سخت‌گیرانه، استریم TTS جمله‌به‌جمله، barge-in فول‌دوبلکس با کالیبراسیون اتاق-ساکت، فیلتر توهم Whisper، پاسخ صوتی گیت‌وی در تلگرام/دیسکورد با سه حالت و DM/کانال، کانال‌های صوتی دیسکورد ‏(مجوزها + intents + opus + کنترل دسترسی + جلوگیری از اکو)، مرجع کامل config.yaml و env، جداول مقایسهٔ STT و TTS و عیب‌یابی
- زنجیرهٔ pager: …extending-the-dashboard ← voice-mode ← (به‌زودی)
- پیشرفت: **۹۴ از ۲۰۷ صفحه فعال (۴۵٪)**

### نشست ۵۹ — 2026-08-26 — 🔊 صدا و TTS — رسانه و وب ۴/۵
- ترجمه و انتشار «صدا و TTS» (۷ بخش، دومرحله‌ای): جدول ۱۱ ارائه‌دهندهٔ TTS، تحویل پلتفرمی، پیکربندی کامل yaml شامل قواعد ناحیه‌ای MiniMax و تگ‌های صوتی Gemini/xAI و پرامپت‌های شخصیت Gemini، محدودیت‌های طول ورودی با سقف آگاه-از-مدل ElevenLabs و بازنویسی per-provider، حباب‌های تلگرام و ffmpeg، صداهای سفارشی xAI، Piper محلی با ۴۴ زبان، ارائه‌دهنده‌های نوع-فرمان TTS ‏(placeholder ها، env_passthrough، مثال Doubao، یادداشت‌های رفتاری و امنیت)، پلاگین‌های پایتونی TTS با هوک‌های اختیاری؛ بخش STT: رونویسی خودکار پیام صوتی، جزئیات ۸ ارائه‌دهنده، رفتار fallback با انتخاب صریح، ارائه‌دهنده‌های فرمان STT ‏(whispercpp/parakeet/sensevoice)، ترتیب حل چهارمرحله‌ای و پلاگین‌های پایتونی STT
...
### نشست ۷۳ — 2026-08-27 — 🧠 حافظهٔ Honcho — قابلیت‌های اصلی دوباره کامل ‏(۳۳/۳۳)
- ترجمه و انتشار «حافظهٔ Honcho» (۴ بخش): جدول مقایسه با حافظهٔ داخلی، استدلال جدلی چند-پاسی ‏(پاس‌های cold-warm/خود-ممیزی/آشتی‌دهی با bail زودهنگام)، سه پیچ مستقل، prewarm ی شروع-نشست و سطح استدلال تطبیقی-با-کوئری؛ معماری دولایه و مرجع کامل config؛ نگاشت هویت گیت‌وی با سه سناریو و نردبان resolver و هشدار یتیم‌شدن حافظه هنگام un-pin؛ observation دو-حالته با الگوهای رایج؛ پنج ابزار و جدول کامل CLI؛ مهاجرت از hermes honcho قدیمی
...
### نشست ۸۰ — 2026-08-27 — 💡 نکات و بهترین شیوه‌ها — صفحهٔ آغازین راهنماها فارسی شد
- ترجمه و انتشار «نکات و بهترین شیوه‌ها» ‏(۷ بخش): گرفتن بهترین نتیجه‌ها ‏(پرامپت دقیق، زمینه از اول، AGENTS.md، رهاکردن ابزارها، مهارت‌ها)؛ نکات حرفه‌ای CLI ‏(ورودی چندخطی با Alt+Enter/Ctrl+J، تشخیص paste، قطع با Ctrl+C، ازسرگیری ‎-c‎/‎-r‎، چسباندن تصویر، تکمیل Tab، چرخهٔ verbose)؛ فایل‌های زمینه ‏(AGENTS.md، SOUL.md با نمونه، سازگاری cursorrules.، کشف lazy ی زیرپوشه‌ها)؛ حافظه و مهارت‌ها ‏(چه در برابر چگونه، ظرفیت ۲۲۰۰/۱۳۷۵ کاراکتری، اسنپ‌شات منجمد)؛ کارایی و هزینه ‏(کش پرامپت نشکن، ‎/compress‎، واگذاری موازی delegate_task، execute_code ی دسته‌ای، انتخاب مدل درست، hermes prompt-size)؛ نکات پیام‌رسانی ‏(/sethome، ‏/title، جفت‌سازی DM، verbose)؛ امنیت ‏(Docker برای کد نامطمئن، دام انکودینگ ویندوز cp125x، چهارگزینهٔ تأیید فرمان خطرناک، skip ی چک‌ها روی کانتینر، allowlist ها)
- پیشرفت: **۱۳۱ از ۲۰۷ صفحه فعال (۶۳٪)**
### نشست ۸۱ — 2026-08-27 — 📚 راهنماها ۲ — شش صفحه؛ MiniMax OAuth و مهاجرت OpenClaw فارسی شدند
- ترجمه و انتشار «اجرای LLM های محلی روی مک» ‏(دو بک‌اند llama.cpp/omlx با جدول مقایسه، انتخاب Qwen3.5-9B با قاعدهٔ سرانگشتی حافظه، فلگ‌های کش KV ی کوانتیزه q4_0 با جدول صرفه‌جویی ۷۵٪، بنچمارک کامل TTFT/تولید روی M5 Max، جدول انتخاب سناریو، تایم‌اوت‌های استریم و prefill)؛ «بات خلاصهٔ روزانه» ‏(جریان پنج‌گامی، تست دستی، قاعدهٔ طلایی پرامپت خودبسنده با نمونهٔ بد/خوب، خلاصهٔ چندموضوعی، واگذاری موازی، زمان‌بندی روزهای هفته و دوبار-در-روز، زمینهٔ شخصی در پرامپت)؛ «پایپ خروجی اسکریپت به پیام‌رسان‌ها» ‏(hermes send با مرجع کامل فلگ‌ها، پنج قالب هدف، کدهای خروج، ترتیب حل بدنه، چهار نمونهٔ واقعی پایش/CI/cron/کار بلند، ‎--json/--quiet‎، بی‌نیازی از گیت‌وی برای توکن-بات، فهرست مقاصد از channel_directory.json)؛ «Cron فقط-اسکریپتی» ‏(حالت بدون-ایجنت با دیاگرام زمان‌بند←اسکریپت←تحویل، ساخت از چت با transcript نمونه، تصمیم خودکار no_agent، نگاشت خروجی به تحویل با دروازهٔ wakeAgent، قوانین پوشهٔ scripts و مفسر پسوندمحور، مثال کامل هشدار دیسک، مقایسه با OS cron + وب‌هوک)؛ «MiniMax OAuth» ‏(جدول نگاه کلی، جریان PKCE پنج‌مرحله‌ای، منطقهٔ چین با minimax-cn، ‎--no-browser‎، مرجع config و نام‌های مستعار، دو مدل با سقف ۲۰۰k، شش عیب‌یابی شامل قرنطینهٔ refresh token)؛ «مهاجرت از OpenClaw» ‏(hermes claw migrate با هشت گزینه، چهارده جدول نگاشت کامل از شخصیت/مهارت‌ها/رفتار ایجنت/MCP/TTS تا پلتفرم‌های پیام‌رسانی، آرشیو چهارده فایلی، حل کلید API از چهار منبع با allowlist، مدیریت SecretRef سه‌قالبی، چک‌لیست هشت‌گامی بعد از مهاجرت)
...
| 131 | `guides/tips.html` | نکات و بهترین شیوه‌ها |
| 132 | `guides/local-llm-on-mac.html` | اجرای LLM های محلی روی مک |
| 133 | `guides/daily-briefing-bot.html` | آموزش: ساخت بات خلاصهٔ روزانه |
| 134 | `guides/pipe-script-output.html` | پایپ کردن خروجی اسکریپت به پیام‌رسان‌ها |
